...
for LINEA in $(cat ${FICHEROMAQUINAS} | grep Windows); do
	...
	# Renomeado do equipo
	NOME=$(vboxmanage guestcontrol ${MAQUINA} \
		--username administrador \
		--password ${CONTRASINAL} \
		run  ${POWERSHELL} \
		-- -command Invoke-Expression -Command 'hostname' |  tr -d '\r\n')

 	if [ "${NOME}" = "${MAQUINA}" ]; then 
		echo A máquina xa foi configurada previamente.
	else 
		vboxmanage guestcontrol ${MAQUINA} \
			--username administrador \
			--password ${CONTRASINAL} \
...
for LINEA in $(cat ${FICHEROMAQUINAS} | grep Ubuntu); do
	MAQUINA=$(echo ${LINEA} | cut -d : -f 1)
	IP=$(echo ${LINEA} | cut -d : -f 3)
	USUARIO=$(echo ${LINEA} | cut -d : -f 4)
	CONTRASINAL=$(echo ${LINEA} | cut -d : -f 5)
...
for LINEA in $(cat ${FICHEROMAQUINAS} | grep Debian); do
	MAQUINA=$(echo ${LINEA} | cut -d : -f 1)
	IP=$(echo ${LINEA} | cut -d : -f 3)
	USUARIO=$(echo ${LINEA} | cut -d : -f 4)
	CONTRASINAL=$(echo ${LINEA} | cut -d : -f 5)
